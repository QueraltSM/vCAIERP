<div class="col-12">
    <?php
    $rss = simplexml_load_file("https://www.boe.es/rss/canal_leg.php?l=l&c=128");
    ...
    foreach ($rss->channel->item as $item) {
        ...
        if (count($matches_eliminar) == 0 && $count < 3) { // && count($matches_aceptar) > 0 
            $entidad = trim(explode('-', $item->description)[0]);
    ?>
            <p style="font-size:14px;text-align:justify;">
                <span class="badge"><i class="bi bi-calendar"></i>&nbsp;&nbsp;<?php echo parse_rss_fecha($item->pubDate) . " - " . $entidad; ?></span><br>
                <?php
